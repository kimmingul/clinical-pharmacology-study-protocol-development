...
`${CLAUDE_PLUGIN_ROOT}/references/templates/inclusion_exclusion_criteria.md`를 Read하여 표준 템플릿을 사용자에게 제시한다.
...
| 시험 유형 | 주요 설계 옵션 |
|----------|--------------|
| DDI (**단방향**) | **One-sequence** (고정순서, MBI·장반감기 perpetrator), **Crossover 2×2**, Parallel |
| DDI (**양방향**) | **Williams 6×3** (3 treatments: A 단독, B 단독, A+B), Parallel 2-arm |
...
### ★ DDI 시험의 방향성 결정 (연구설계 확정 전 선결)

DDI 시험이면 연구설계를 확정하기 **전에** 다음 절차로 방향성을 결정한다. 이 단계를 생략하면 단방향 기본값이 암묵적으로 채택되어 양방향 평가가 필요한 시험에서 결함이 발생한다.

1. **`_workspace/01_research_cp.md`의 "DDI 방향성 분석" 섹션을 Read**하여 clinical-pharmacologist가 작성한 역할 매트릭스·방향성 판정·권고를 확인
2. **사용자에게 명시적으로 확인**:
   > "이 시험은 **단방향 DDI**(예: Y가 X의 PK에만 영향) 평가입니까, **양방향 DDI**(X↔Y 상호 영향) 평가입니까?"
3. 판단 기준 제시:
   - 두 약물 모두 공통 CYP/수송체의 **기질**이고 각자 상대 약물 대사에 **저해/유도 효과**가 있으면 → **양방향 권장**
   - 한 약물만 명확한 perpetrator(강한 저해/유도제이나 비기질)이면 → **단방향** 충분
   - 비가역적 저해제(MBI, 예: Omeprazole의 CYP2C19 MBI)이면 carry-over 제어가 핵심 — one-sequence 선호
   - 규제 전략(FDA/EMA/MFDS·의뢰자 목적)에 따라 요구 방향이 다를 수 있음
4. 결정을 `design_decisions.md §연구설계`에 **"방향성: 단방향(Y→X) / 단방향(X→Y) / 양방향(X↔Y)"**로 명시

#### 방향성별 설계 상세

**단방향 DDI**:
- One-sequence 권장: perpetrator가 MBI 또는 긴 반감기
- 2×2 crossover 권장: perpetrator 가역적 + washout 충분
- 1차 평가변수: victim 약물의 AUC/Cmax GMR (perpetrator PK는 2차)
- Sample size 스크립트: `one_sequence_ddi.py` 또는 `crossover_2x2_ddi.py`

**양방향 DDI — Williams 6×3 crossover (대표)**:
- 3 treatments: **A = X 단독**, **B = Y 단독**, **C = X+Y 병용**
- 6 sequences = 3!(ABC, ACB, BAC, BCA, CAB, CBA) → **1차 carry-over 균형**
- 3 periods per subject
- 1차 평가변수 (co-primary):
  - **Direction 1**: A vs C — X의 AUC/Cmax GMR (Y의 영향)
  - **Direction 2**: B vs C — Y의 AUC/Cmax GMR (X의 영향)
- 통계: **Intersection-Union Test (IUT)** — 두 방향 모두 90% CI ⊂ (0.80, 1.25) 충족 시 "상호작용 없음" 판정, **α 조정 불필요**
- Sample size 스크립트: `${CLAUDE_PLUGIN_ROOT}/scripts/sample_size/williams_6x3_ddi.py` (두 방향 각각 검정력 계산 → 더 큰 n 채택 → 6의 배수로 올림)
- Carry-over 리스크: MBI가 있으면 washout 연장 또는 parallel 2-arm 대안

**양방향 대안 — Parallel 2-arm**: Williams 6×3이 carry-over로 불가할 때
- Arm 1: X 단독 → X+Y (X의 PK 변화 측정)
- Arm 2: Y 단독 → Y+X (Y의 PK 변화 측정)
- 각 arm 독립 단방향 시험 병행
...
Agent(
  ...
  prompt: "먼저 ${CLAUDE_PLUGIN_ROOT}/agents/biostatistician.md를 Read하여 역할을 숙지하라.
설계 결정: _workspace/00_input/design_decisions.md를 Read하라.
배경 자료: _workspace/01_research_report.md를 Read하라.
${CLAUDE_PLUGIN_ROOT}/scripts/sample_size/ 내 해당 Python 코드를 Read하여 파라미터를 채워 실행하라.
