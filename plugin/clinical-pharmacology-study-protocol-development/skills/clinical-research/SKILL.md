...
| **약물유전체학** (CYP/표적 다형성, 한국인 빈도) | translational-scientist | **PharmGKB + CPIC (WebFetch)** + PubMed (한국인 빈도) — `${CLAUDE_PLUGIN_ROOT}/references/api_reference/{pharmgkb,cpic}.md` |
| **대사체학** (인체 특이 대사체, 내인성 바이오마커) | translational-scientist | PubMed |
| 규제 가이드라인 | regulatory-expert | MFDS/FDA/EMA 가이드라인 DB |
| 약물 라벨 정보 (PG 섹션 추출 포함) | regulatory-expert | **DailyMed + openFDA (WebFetch)** — `${CLAUDE_PLUGIN_ROOT}/references/api_reference/{dailymed,openfda}.md` |
| ICD-10 코딩 | regulatory-expert | ICD-10 API |
| MFDS 승인현황 | regulatory-expert | **MFDS 의약품안전나라 (WebFetch)** — `nedrug.mfds.go.kr/searchClinic`, `${CLAUDE_PLUGIN_ROOT}/references/api_reference/mfds.md` |
...
- `${CLAUDE_PLUGIN_ROOT}/scripts/fih/starting_dose_calculation.py`를 참조하여 코드로 수행
...
**규제 가이드라인 참조 (사전 수록):**
- 시작점: `${CLAUDE_PLUGIN_ROOT}/references/guidelines/index.md` — 전체 인덱스 (ICH/FDA/EMA/MFDS, 국내 법령, 시험 유형별 cross-agency 비교)
- MFDS: `${CLAUDE_PLUGIN_ROOT}/references/guidelines/mfds/` (BE, DDI, FIH, 임상약리 일반, 통계)
- FDA: `${CLAUDE_PLUGIN_ROOT}/references/guidelines/fda/` (BE, DDI, FIH, FE, QTc, BMV)
- EMA: `${CLAUDE_PLUGIN_ROOT}/references/guidelines/ema/` (BE, DDI, FIH, BMV)
- ICH: `${CLAUDE_PLUGIN_ROOT}/references/guidelines/ich/` (E6 R3, E8 R1, E14, M13A)
- Cross-Agency 비교: `${CLAUDE_PLUGIN_ROOT}/references/guidelines/by_study_type/` — 시험 유형별 4개 기관 차이점 통합 비교
- 국내 법령: `${CLAUDE_PLUGIN_ROOT}/references/guidelines/regulations/` (약사법, KGCP, PIPA, 생명윤리법)

### Step 3: 약물 라벨 정보 (허가 약물) — WebFetch 기반

허가된 약물의 경우 **DailyMed (1차)** + **openFDA (보완)** WebFetch로 라벨·허가 정보 조회. 상세 쿼리 레시피는 `${CLAUDE_PLUGIN_ROOT}/references/api_reference/{dailymed,openfda}.md` 참조.
...
상세 쿼리 레시피·HTML 파싱 힌트·Python 파싱 코드는 **`${CLAUDE_PLUGIN_ROOT}/references/api_reference/mfds.md`** (정본) 참조. 아래는 핵심만 요약.
...
- **clinician은 항상 참여**: 건강인 대상(DDI, BE, FE)이든 환자 대상(FIH, Special Pop, QTc)이든 **모든 시험**에서 참여한다. 안전성 프로파일의 체계적 수집·검토는 시험 유형과 무관하게 필수이기 때문이다 (clinician.md, trial-doc-orchestrator/SKILL.md와 일치). BE/FE에서 불참하는 것은 **translational-scientist**다 — 혼동 금지
