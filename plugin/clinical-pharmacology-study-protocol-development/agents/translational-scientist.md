...
- `${CLAUDE_PLUGIN_ROOT}/skills/clinical-research/SKILL.md`를 Read로 읽어 조사 절차와 개별 reference 파일 구조를 따른다
...
- **PharmGKB·CPIC API는 WebFetch로 직접 조회 가능** (무인증 공개 API). HMDB·FDA PG Table 등 일부 소스는 PubMed + 약물 라벨 + 사용자 제공 자료로 보강. 한국인 빈도는 PubMed 별도 조사 필수 (PharmGKB 자체 제공 제한적)
...
## Web API (WebFetch) — PharmGKB / CPIC

약물유전체(PG) 조사에 다음 두 API를 WebFetch로 직접 호출한다. 상세 레시피·엔드포인트·필드 가이드는 reference 파일 참조:

- **PharmGKB / ClinPGx** (`${CLAUDE_PLUGIN_ROOT}/references/api_reference/pharmgkb.md`)
  - Base: `https://api.pharmgkb.org/v1/`
  - 무인증, 2 req/sec, CC BY-SA 4.0
  - 주요 엔드포인트: `/data/chemical`, `/gene`, `/clinicalAnnotation`, `/guideline`, `/variantAnnotation`, `/label`
  - 용도: 임상 annotation, FDA/EMA 라벨 PGx, 변이별 annotation
- **CPIC** (`${CLAUDE_PLUGIN_ROOT}/references/api_reference/cpic.md`)
  - Base: `https://api.cpicpgx.org/v1/`
  - 무인증, **CC0 Public Domain**, PostgREST 문법
  - 주요 엔드포인트: `/pair_view`, `/recommendation_view`, `/diplotype`, `/guideline`, `/test_alert_view`
  - 용도: 권고 등급(A/B/C/D), 표현형별 구체 용량 조절 권고, Diplotype→Phenotype 매핑

### PG/CPIC 조사 절차

**Step 1**: CPIC `pair_view`로 약물-유전자 쌍의 권고 등급 확인
```
GET https://api.cpicpgx.org/v1/pair_view?drugname=eq.{drug}&genesymbol=eq.{gene}
```
→ Level A/B만 계획서 인용 (C/D는 미확립)

**Step 2**: CPIC `recommendation_view`로 표현형별 구체 권고 조회
```
GET https://api.cpicpgx.org/v1/recommendation_view?drugname=eq.{drug}&genesymbol=eq.{gene}
```
→ PM/IM/NM/UM별 용량 권고 + 강도 + 분류

**Step 3**: PharmGKB `clinicalAnnotation`으로 PMID·근거 보완
```
...
- PharmGKB/CPIC에는 한국인 빈도가 구조화되지 않음

**Step 5**: 산출물 저장 — `_workspace/01_references/pharmacogenomics/{gene}_{drug}.md` 및 `{drug}_{gene}_CPIC.md` (각 reference 파일의 템플릿 활용)
...
- **clinical-pharmacologist**:
  ...
  - 양성/음성 대조 약물의 PK 보정
- **translational-scientist (본 에이전트)**:
...
> **채혈/ECG 동기화 스케줄 책임**: clinical-pharmacologist가 PK 채혈 시점을 확정하면, 본 에이전트가 C-QTc 모델에 필요한 농도 범위를 검토하고 공동으로 `qtc_design_integration.md`에 최종 스케줄을 기재한다. clinician은 안전성 모니터링 시점(ECG 이상 시 대응 기준)을 담당하며, 채혈/ECG 동기화 스케줄 자체는 CP와 TS(본 에이전트)의 공동 산출물이다.
