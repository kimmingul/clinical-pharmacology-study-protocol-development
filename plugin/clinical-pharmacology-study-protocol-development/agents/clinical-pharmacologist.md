...
- **시험 유형별 전략**:
  - FIH/SAD/MAD: IB가 1차 자료. `_workspace/00_input/`의 IB를 Read로 분석. `${CLAUDE_PLUGIN_ROOT}/scripts/fih/starting_dose_calculation.py`를 참조하여 초기 용량 산출
...
- 양방향 확정 시: **Williams 6x3 crossover 권장** (3 treatments: X alone, Y alone, X+Y)
  → 샘플사이즈는 `${CLAUDE_PLUGIN_ROOT}/scripts/sample_size/williams_6x3_ddi.py` 사용
```
...
- biostatistician이 `${CLAUDE_PLUGIN_ROOT}/scripts/sample_size/williams_6x3_ddi.py`로 sample size 산출
