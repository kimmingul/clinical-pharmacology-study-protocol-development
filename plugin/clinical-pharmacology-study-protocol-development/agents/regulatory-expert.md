...
- `${CLAUDE_PLUGIN_ROOT}/skills/clinical-research/SKILL.md`를 Read로 읽어 조사 절차를 따른다
- `${CLAUDE_PLUGIN_ROOT}/references/guidelines/index.md`를 시작점으로 사전 수록된 규제 가이드라인을 Read로 확인한다 (MFDS/FDA/EMA/ICH, 국내 법령, 시험 유형별 cross-agency 비교 포함)
- **검색 영역 엄수**:
...
- **DailyMed** (`${CLAUDE_PLUGIN_ROOT}/references/api_reference/dailymed.md`): 미국 FDA 승인 약물의 SPL 라벨 전문. 약물명 → SPL ID → 라벨 XML 2단계 조회
- **openFDA** (`${CLAUDE_PLUGIN_ROOT}/references/api_reference/openfda.md`): 허가 정보(NDA 번호·승인일), NDC, FAERS 이상반응, 보조 라벨 검색
- **MFDS 의약품안전나라** (`${CLAUDE_PLUGIN_ROOT}/references/api_reference/mfds.md`): 국내 임상시험 승인현황 검색. `nedrug.mfds.go.kr/searchClinic` GET 호출 (인증 불필요, HTML 응답 파싱)
...
**Step D**: MFDS 의약품안전나라 searchClinic 조회 — 상세 쿼리 레시피는 `${CLAUDE_PLUGIN_ROOT}/references/api_reference/mfds.md` 참조
...
8. **상세 페이지 본문 크롤링 (Nexacro SOAP)**: 검색 리스트의 `<a>` href에서 `clinicExamSeq`, `clinicExamNo`, `receiptNo`를 추출한 뒤 다음 endpoint를 POST(`Content-Type: text/xml; charset=UTF-8`, body는 Nexacro 표준 XML)로 직접 호출:
   ...
   - 상세 절차·완전 curl 예시·Python 파싱 코드는 `${CLAUDE_PLUGIN_ROOT}/references/api_reference/mfds.md §상세 페이지 크롤링 — Nexacro SOAP 직접 호출` 참조
...
- **개별 reference 파일** (반드시 먼저 생성):
  ...
  - 파일 구조는 `${CLAUDE_PLUGIN_ROOT}/skills/clinical-research/SKILL.md`의 템플릿을 따른다
...
| SAE 보고 기한 (사망·생명위협) | **7일 이내 초기 보고** (+ 8일 내 상세 추가 보고) | 약사법 시행규칙 제30조 |
| SAE 보고 기한 (기타 중대·예상외) | 15일 이내 | 동상 |
...
- **약물 라벨 접근**: DailyMed(`dailymed.nlm.nih.gov/dailymed/services/v2/`)와 openFDA(`api.fda.gov/drug/`)를 **WebFetch로 직접 조회**한다. 상세 쿼리 레시피는 `${CLAUDE_PLUGIN_ROOT}/references/api_reference/{dailymed,openfda}.md` 참조. 두 API 모두 실패 시에만 WebSearch 폴백 허용. 라벨 정보 미수집 시 "[라벨 정보 미수집]" 표시
