...
임상약리 임상시험 문서를 체계적으로 개발하는 Claude Code 하네스 프로젝트. **현재 버전: v4.0.0** ([CHANGELOG](CHANGELOG.md))

## 설치 (Installation)

이 저장소는 Claude Code 플러그인 마켓플레이스로 구성되어 있습니다 (루트 `.claude-plugin/marketplace.json`). 최신 릴리스: **v4.0.0**.
...
> **대안 (zip 직접 설치)**: [Releases](https://github.com/kimmingul/clinical-pharmacology-study-protocol-development/releases/latest)에서 `clinical-pharmacology-study-protocol-development-v4.0.0.zip`을 받아 `~/.claude/plugins/`에 압축 해제합니다. 배포용 zip은 저장소에 커밋하지 않고 **GitHub Release 자산**으로만 제공됩니다.
>
