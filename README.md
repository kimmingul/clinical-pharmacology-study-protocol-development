...
임상약리 임상시험 문서를 체계적으로 개발하는 Claude Code 하네스 프로젝트. **현재 버전: v4.3.0** ([CHANGELOG](CHANGELOG.md))

## 설치 (Installation)

이 저장소는 Claude Code 플러그인 마켓플레이스로 구성되어 있습니다 (루트 `.claude-plugin/marketplace.json`). 최신 릴리스: **v4.3.0**.
...
> **대안 (zip 직접 설치)**: [Releases](https://github.com/kimmingul/clinical-pharmacology-study-protocol-development/releases/latest)에서 `clinical-pharmacology-study-protocol-development-v4.3.0.zip`을 받아 `~/.claude/plugins/`에 압축 해제합니다. 배포용 zip은 저장소에 커밋하지 않고 **GitHub Release 자산**으로만 제공됩니다.
>
...
> **v4.3.0 (fail-closed release 게이트)**: `/finalize`의 판정 주체를 에이전트 해석에서 **결정적 실행 파일**(`.claude/scripts/qa/finalize_run.py`)로 이전. 5개 차원(`structure`/`citation`/`dose`/`advisory`/`approval`)을 실행하고 **종료코드로만** 판정한다(`0` 통과 / `1` 불합격 / `2` 판정 불가, `ERROR`가 `FAIL`보다 우선). `<workspace>/verification/release_gate.json`에 증거를 원자적으로 기록하며, **기록에 실패하면 통과를 주장하지 않는다**. 미구현 통제(사람 승인)는 숨기지 않고 `NOT_IMPLEMENTED`로 매 실행 노출된다. 개발 중 fail-open 6건을 실측 재현 후 차단. ([CHANGELOG](CHANGELOG.md#430--2026-07-31--fail-closed-release-게이트))
>
...
Phase 4: 대화형 설계 협의
    ...
    Step 3 세부 요소:
            ...
            - 안전성 평가 항목
            - 유전체/대사체 분석 계획 (BE/FE 외 시험; ICF Part 4와 연계)
            - washout 기간, 투여 조건 등
         │
         ▼
