...
> **v3.0.0 업데이트 (2026-06-15)**: §0(0-1·0-3)은 v2.1.0까지 종결, §3(에이전트 모델 재검토)은 **v3.0.0에서 전 에이전트 opus 전환으로 종결**. §4·§5(WebFetch→MCP/`data.go.kr`)는 v3 제안서(`docs/plugin_v3_advancement_proposal_ko.md` §6 후속)와 통합 추적. 잔존 활성 항목: §0-2(PharmGKB Minor), §1, §2, §4, §5, §6.
...
## 3. ~~biostatistician / clinician 모델 재검토~~ ✅ **종결 (v3.0.0, 2026-06-15)**

> v3.0.0에서 **전 에이전트(8종)를 opus(최신)로 전환**하여 본 항목은 무의미해졌다. 아래는 히스토리 보존용.
