...
$PY .claude/scripts/llm/health_check.py --workspace _workspace
...
$PY .claude/scripts/llm/route_select.py --host "$HOST" --workspace _workspace
...
- 모든 외부 모델 호출은 `.claude/scripts/llm/ask_model.sh`를 통해서만 한다(직접 CLI 호출 금지). 어댑터가 `egress_gate.py`로 **데이터 분류 → 허용 provider**를 fail‑closed로 검사한다.
