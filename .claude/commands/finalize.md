...
$PY .claude/scripts/qa/doc_lint.py "$TARGET" --strict ${GOAL:+--goal-spec "$GOAL"}; LINT=$?

# (b) 채점 (참고용 — score≥90 권장)
$PY .claude/scripts/qa/doc_lint.py "$TARGET" --score ${GOAL:+--goal-spec "$GOAL"}

# (c) zero-trust 인용 검증
$PY .claude/scripts/qa/citation_verify.py audit "$TARGET" --workspace "$WS"

# (d) FIH 용량 안전 가드 — protocol 대상일 때만 (ICF는 생략)
DOSE=0
if [ "$IS_ICF" = "0" ]; then
  $PY .claude/scripts/qa/dose_safety_guard.py "$TARGET" \
