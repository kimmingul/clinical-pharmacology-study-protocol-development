...
> .claude/scripts/llm/run_review_panel.sh --workspace _workspace $CLASS \
...
1. **결정적 채점 먼저(critic A)** — Bash로 실행하여 객관 점수를 얻는다:
   ...
   $PY .claude/scripts/qa/doc_lint.py _workspace/03_protocol_draft.md --score \
       ${GOAL:+--goal-spec "$GOAL"}
   ```
