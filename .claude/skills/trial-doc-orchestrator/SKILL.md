...
description: "임상약리 임상시험 문서(계획서, 동의설명서, 동의서, 개인정보 동의서) 개발을 조율하는 오케스트레이터. 7개 전문 에이전트를 10단계 파이프라인으로 실행한다. 임상시험 문서 작성, 계획서 개발, 프로토콜 개발, 동의설명서 개발, 시험 문서 생성 요청 시 사용. 후속 작업: 문서 수정, 계획서 수정, 동의설명서 수정, 프로토콜 업데이트, 문서 보완, 다시 실행, 재실행, 특정 섹션만 다시, 이전 결과 개선, QA 피드백 반영 요청 시에도 반드시 이 스킬을 사용."
---

# Trial Document Orchestrator

7개 전문 에이전트와 10단계 워크플로우로 임상약리 임상시험 문서를 개발한다. 메인 에이전트가 전체를 조율하고, 사용자 검토 게이트를 통해 단계 진행을 제어한다.
...
| 에이전트 | model | 역할 | 스킬 |
|---------|-------|------|------|
| clinical-pharmacologist | sonnet | PK/PD, 대사 경로, 약물상호작용, 용량 근거, FIH 초기 용량 | clinical-research |
| regulatory-expert | sonnet | MFDS/FDA 가이드라인, 약물 라벨, ICD-10 | clinical-research |
| clinician | sonnet | 선정/제외 기준, 안전성, 임상 절차 (**조건부**) | clinical-research |
| biostatistician | sonnet | 연구설계 옵션, sample size 계산 | — (Phase 5에서 직접) |
...
> **실행 방식**: 모든 에이전트는 `general-purpose` 타입으로 호출한다. 에이전트 정의(`.claude/agents/*.md`)와 스킬을 Read로 로드하는 방식이다.
...
두 에이전트를 **병렬로** 호출한다. 검색 영역이 분리되어 중복이 없다.
...
Agent(
  ...
  prompt: "먼저 .claude/agents/clinical-pharmacologist.md를 Read하여 역할과 원칙을 숙지하라.
그 다음 .claude/skills/clinical-research/SKILL.md를 Read하여 조사 절차를 따르라.
...
산출물을 _workspace/research_clinical_pharmacologist.md에 Write하라."
)
```
...
Agent(
  ...
  prompt: "먼저 .claude/agents/regulatory-expert.md를 Read하여 역할과 원칙을 숙지하라.
그 다음 .claude/skills/clinical-research/SKILL.md를 Read하여 조사 절차를 따르라.
...
산출물을 _workspace/research_regulatory_expert.md에 Write하라."
)
```

**조건부: clinician 호출 (환자 대상 시험 시)**

FIH/SAD/MAD, QTc, PK Special Population 등 환자 대상 시험에서만 호출한다:
```
Agent(
  description: "임상적 판단 및 안전성 관련 자료 수집",
  model: "sonnet",
  name: "clinician",
  prompt: "먼저 .claude/agents/clinician.md를 Read하여 역할과 원칙을 숙지하라.
그 다음 .claude/skills/clinical-research/SKILL.md를 Read하여 조사 절차를 따르라.
...
산출물을 _workspace/research_clinician.md에 Write하라."
)
```
...
1. 메인 에이전트가 각 에이전트의 산출물을 Read로 읽는다:
   - `_workspace/research_clinical_pharmacologist.md`
   - `_workspace/research_regulatory_expert.md`
   - (해당 시) `_workspace/research_clinician.md`
...
> 이 단계는 **서브 에이전트가 아닌 메인 에이전트가 사용자와 직접 대화**하며 설계 결정을 내린다.

수집된 자료를 기반으로 설계 옵션을 제시하고, 사용자와 함께 결정한다:

1. **연구설계 옵션**: 시험 유형에 따른 후보 설계를 장단점과 함께 제시
   - Crossover 유형: one-sequence, 2x2, 2x3, 2x4, 4x4 (Williams), 6x3
   - Parallel, Adaptive 등
   - 유사 시험 사례 참조하여 근거 제시
2. **PK 채혈 시점**: 예상 반감기 기반 시점 설계
3. **평가변수**: 1차/2차/탐색적
4. **안전성 평가 항목**: 활력징후, ECG, 실험실 검사 등
5. **기타**: washout 기간, 투여 조건, 식이 조건 등

각 옵션에 대해 사용자 결정을 받은 뒤 Phase 5로 진행한다.
...
Agent(
  ...
  prompt: "먼저 .claude/agents/biostatistician.md를 Read하여 역할과 원칙을 숙지하라.
...
.claude/scripts/sample_size/ 디렉토리의 해당 코드 템플릿을 Read하여 파라미터를 채워 Bash로 실행하라.
...
Agent(
  ...
  prompt: "먼저 .claude/agents/protocol-writer.md를 Read하여 역할과 원칙을 숙지하라.
그 다음 .claude/skills/protocol-drafting/SKILL.md를 Read하여 작성 가이드를 따르라.
상세 템플릿이 필요하면 .claude/skills/protocol-drafting/references/protocol-template.md도 Read하라.
...
4명의 전문가가 **병렬로** 계획서를 리뷰한 후, qa-reviewer가 취합한다.
...
Agent(
  ...
  model: "sonnet",
  name: "{agent-name}",
  prompt: "먼저 .claude/agents/{agent-name}.md를 Read하여 역할과 원칙을 숙지하라.
그 다음 .claude/skills/regulatory-review/SKILL.md를 Read하여 '개별 리뷰어 절차' 섹션을 따르라.
...
| 에이전트 | 초점 |
|---------|------|
| clinical-pharmacologist | PK 설계, 채혈 시점, 약물상호작용 평가, 용량 근거, washout |
| clinician | 선정/제외 기준, 안전성 모니터링, 이상반응 관리, 중지 기준 (**조건부**) |
| regulatory-expert | ICH E6(R3) Annex 1 필수 요소, MFDS/FDA 준수, 규제 용어 |
| biostatistician | 통계 섹션, sample size, 무작위화, 분석 방법, 결측치 처리 |
...
- **Phase 2에서 에이전트 검색 영역이 겹치면 안 된다**: clinical-pharmacologist는 PubMed/ClinicalTrials.gov, regulatory-expert는 가이드라인/라벨/ICD-10으로 엄격히 분리
- **clinician 호출 조건**: 건강한 성인 대상 시험(DDI, BE, FE)에서는 불참. 환자 대상(FIH, QTc, Special Pop)에서만 참여
