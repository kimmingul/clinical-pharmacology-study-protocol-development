...
### MFDS 국내 임상시험 승인현황 조사 절차

**Step D**: MFDS 의약품안전나라 searchClinic 조회
1. 약물명/의뢰자/적응증 기반 GET 요청:
   `https://nedrug.mfds.go.kr/searchClinic?searchYn=true&page=1&searchType={ST1|ST3|ST4}&searchKeyword={keyword}&approvalDtStart={YYYY-MM-DD}&approvalDtEnd={YYYY-MM-DD}`
2. HTML 응답에서 "총 N건", 시험 제목, 의뢰자, 단계, 실시기관, 승인일 추출
3. 페이지네이션 필요 시 `page` 증가 (과도한 호출 주의)
4. 산출물: `_workspace/01_references/mfds_clinical_trials/{topic}_approval_list.md`
5. 상세 쿼리 레시피·파라미터는 `.claude/references/api_reference/mfds.md` 참조
